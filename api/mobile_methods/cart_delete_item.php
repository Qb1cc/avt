<?php
//Скрипт для метода удаления позиции корзины
...
if( $session_query->fetchColumn() > 0 )
{
	...
	//Проверяем принадлежность позици к пользователю
	...
	if( $check_item_record == false )
	{
		$answer = array();
		$answer["status"] = false;
		$answer["message"] = "Нет такой позиции корзины";
		exit(json_encode($answer));
	}
	...
	if($check_item_record["user_id"] != $user_id)
	{
		$answer = array();
		$answer["status"] = false;
		$answer["message"] = "Ошибка принадлежности позиции корзины";
		exit(json_encode($answer));
	}
	
	
	//Дошли до сюда - значит можно позицию удалять
