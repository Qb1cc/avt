...
try
{
	...
	if( $reg_contact_type == "email" )
	{
		...
		$notify_vars["email_confirm_href"] = "<a target='_blank' href='".$DP_Config->domain_path."users/confirm_contact?code=$activation_code&u_id=$user_id&type=email'>Нажмите на эту ссылку для подтверждения E-mail. <b>Если письма нет во входящих сообщениях, проверяйте папку СПАМ.</b></a>";//Ссылка для подтверждения E-mail
	}
	else
	{
...
if( $reg_contact_type == "email" )
{
	...
	<b>Если письма нет во входящих сообщениях, проверяйте папку СПАМ.</b>";
}
else
{
	?>
