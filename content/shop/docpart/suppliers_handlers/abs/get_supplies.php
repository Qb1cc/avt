...
class abs_enclosure
{
	...
	public function __construct($article, $manufacurers, $storage_options)
	{
		...
		for($m=0; $m < count($manufacurers); $m++)
		{
			...
			$ch = curl_init();
			// установка URL и других необходимых параметров
			curl_setopt($ch, CURLOPT_URL, "http://api.abs-auto.ru/api-search?auth=$hash&article=$article&with_cross=1&agreement_id=$ua_id&format=json&brand=".urlencode($manufacurers[$m]["manufacturer"]));
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($ch, CURLOPT_HEADER, 0);
			// загрузка страницы и выдача её браузеру
			$curl_result = curl_exec($ch);
			...
			if($DocpartSuppliersAPI_Debug->suppliers_api_debug)
			{
				$DocpartSuppliersAPI_Debug->log_api_request("Получение остатков по артикулу ".$article." и производителю ".$manufacurers[$m]["manufacturer"], "http://api.abs-auto.ru/api-search?auth=$hash&article=$article&with_cross=1&agreement_id=$ua_id&format=json&brand=".$manufacurers[$m]["manufacturer"], $curl_result, print_r(json_decode($curl_result, true), true) );
			}
