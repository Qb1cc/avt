...
class abs_enclosure
{
	...
	public function __construct($article, $storage_options)
	{
		...
		$ch = curl_init();
        // установка URL и других необходимых параметров
        curl_setopt($ch, CURLOPT_URL, "http://api.abs-auto.ru/api-search?auth=$hash&article=$article&agreement_id=$ua_id&format=json");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_HEADER, 0);
        // загрузка страницы и выдача её браузеру
        $curl_result = curl_exec($ch);
		...
		if($DocpartSuppliersAPI_Debug->suppliers_api_debug)
		{
			$DocpartSuppliersAPI_Debug->log_api_request("Получение списка товаров по артикулу ".$article, "http://api.abs-auto.ru/api-search?auth=$hash&article=$article&agreement_id=$ua_id&format=json", $curl_result, print_r(json_decode($curl_result, true), true) );
		}
