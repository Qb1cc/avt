...
if ( ! isset( $token ) )
{
	...
	mysqli_query($db_link, "UPDATE `shop_orders_items` SET `sao_state_object`='', `sao_message` = 'Ошибка авторизации при удалении из корзины: ".date("d.m.Y H:i:s", time())."' WHERE `id` = $order_item_id;");
}
else//Работаем дальше - добавляем в корзину
{
	...
	if( $sao_state_object == null )
	{
		...
		//Записываем данные в позицию (строку ответа от поставщика и отображаемый комментарий)
		mysqli_query($db_link, "UPDATE `shop_orders_items` SET `sao_message` = 'Ошибка удаления из корзины: ".date("d.m.Y H:i:s", time())."' WHERE `id` = $order_item_id;");
	}
	else//Есть параметры
	{
		...
		if( $result == "OK" )//ПОЛНЫЙ УСПЕХ
		{
			...
			//Записываем данные в позицию (строку ответа от поставщика и отображаемый комментарий)
			mysqli_query($db_link, "UPDATE `shop_orders_items` SET `sao_message` = 'Удалено из корзины: ".date("d.m.Y H:i:s", time()).". PartID: $PartID', `sao_state` = $new_sao_state, `sao_robot` = 0 WHERE `id` = $order_item_id;");
		}
