...
if ( ! isset( $token ) )
{
	...
	mysqli_query($db_link, "UPDATE `shop_orders_items` SET `sao_state_object`='', `sao_message` = 'Ошибка авторизации при добавлении в корзину: ".date("d.m.Y H:i:s", time())."' WHERE `id` = $order_item_id;");
}
else//Работаем дальше - добавляем в корзину
{
	...
	if($result["StatusCode"] != 200)
	{
		...
		//Записываем данные в позицию (строку ответа от поставщика и отображаемый комментарий)
		mysqli_query($db_link, "UPDATE `shop_orders_items` SET `sao_state_object`='".json_encode($result)."', `sao_message` = 'Ошибка добавления в корзину: ".date("d.m.Y H:i:s", time())."' WHERE `id` = $order_item_id;");
	}
	else//УСПЕШНЫЙ ЗАПРОС
	{
		...
		else//ПОЛНЫЙ УСПЕХ
		{
			...
			//Записываем данные в позицию (строку ответа от поставщика и отображаемый комментарий)
			mysqli_query($db_link, "UPDATE `shop_orders_items` SET `sao_state_object`='".json_encode($result)."', `sao_message` = 'Добавлено в корзину: ".date("d.m.Y H:i:s", time())."', `sao_state` = $new_sao_state, `sao_robot` = 0 WHERE `id` = $order_item_id;");
		}
